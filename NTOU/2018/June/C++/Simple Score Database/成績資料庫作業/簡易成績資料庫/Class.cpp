...
void Class::uniTest(){
	/*streambuf *back, *fileIn;
	ifstream fileStr("uniTestIn.dat");
	back = cin.rdbuf();
	fileIn = fileStr.rdbuf();
	cin.rdbuf(fileIn);*/
	FILE back = *stdin;
	//freopen("uniTestIn.dat", "r", stdin);//會先把fclose(stdin), 再把資料串流stdin改為"uniTestIn.dat"
	FILE *fptr = fopen("uniTestIn.dat", "r");
	*stdin = fptr;
	Class uni;
	uni.query();
	fclose(fptr);//如果用&fptr會不能用
	*stdin = back;
	//cin.rdbuf(back);//freopen("CON", "r", stdin);//還給con表示又要自己輸入
	cout << "\n\n---uniTest成功!\n\n";
}

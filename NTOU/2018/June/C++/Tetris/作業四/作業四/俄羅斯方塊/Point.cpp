...
int Point::hit(const int left, const int top, const int right, const int bottom, int **ground)const{
	if(m_x < left || m_x > right - 1 || m_y > bottom || m_y < top || ground[m_y - top][m_x - left])//right-1因為每一格圖片是由左往右畫
		return -1;
...
void Point::draw(int color)const{
	if(color){
...
void Point::setPixelTable(Rect &boundary, int color){

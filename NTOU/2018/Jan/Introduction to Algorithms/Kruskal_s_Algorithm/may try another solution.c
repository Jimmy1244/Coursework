...
nodes * creatNewNode() {
	nodes * a = (nodes*)malloc(sizeof(nodes));
	a->next = NULL;
	a->last = NULL;
	return a;
}
...
int Kru(edges * edg, int size, int max) {
	...
	for (i = 0; i < size; i++) {
		...
		if (head[edg[i].s] == -1 && head[edg[i].e] != -1) {//大的出現過了
			...
			if (edg[i].e < head[edg[i].s]) {//這種情況應該不會出現
				head[edg[i].e] = edg[i].e;
				head[head[edg[i].s]] = edg[i].e;
				head[edg[i].s] = edg[i].s;
			}
			else {
...
int main() {
	...
	while (scanf("%d%d%d", &edgesA[count].s, &edgesA[count].e, &edgesA[count].credit) != EOF) {
		...
		/*edgesA[count].nod.last = NULL;
		edgesA[count].nod.d = edgesA[count].s;
		edgesA[count].nod.next = (nodes*)malloc(sizeof(nodes));
		edgesA[count].nod.next->last = &edgesA[count].nod;
		edgesA[count].nod.next->d = edgesA[count].e;*///for union指標寫法
		count++;
	}

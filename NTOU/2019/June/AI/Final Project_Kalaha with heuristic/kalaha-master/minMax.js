function MinMaxDecision(depthMax, player) {
	...
	return mValue(house, depthMax, 0, playerShop);
}

function mValue(house, depthMax, depth, playerShop) {
	if (HasSuccessors(house) === false) {
		...
		for (var i = oppoentShop - 6; i < oppoentShop; ++i) {
			...
			if (Relocation(tempHouse, i))
				tempValue = mValue(tempHouse, depthMax, depth + 2, playerShop);
			else
				tempValue = mValue(tempHouse, depthMax, depth + 1, playerShop);
